...
The M5A patch uses ladder drive 0.75. This paired 2× model experiment uses the
frozen reference, cutoff 14,073 Hz, drive 0.75, and all other patch controls
unchanged. The sole intervention disables the PolyBLEP correction before the
same 2× decimator. The metric windows and stage probes are identical between
runs.
...
This confirms that PolyBLEP is necessary for the M5A saw at its actual ladder
drive. The oscillator and mixer continue to agree; the ladder remains the next
stage to investigate for harmonic shape. This is model-only evidence and does
not change the RTL candidate.
