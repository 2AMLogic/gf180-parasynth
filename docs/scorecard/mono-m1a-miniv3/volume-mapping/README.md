...
The three candidate gain errors are **+0.97465, −1.04361 and −0.83170 dB**. No per-note harmonic pass is lost. The tiny aggregate harmonic change remains visible; it does not cross a pass boundary. The baseline (patch `m1a-provisional-v1`) newly rendered WAV exactly reproduces the pre-selection model-audio SHA-256 (`50add9b9…`). **This candidate is now the selected patch, `m1a-gain-minus4db-v2`**, promoted through the scorecard runner; `tools/check_m1a_selection.py` asserts the board record is this candidate (identity, audio hash, property vector) and that the preservation policy above holds. Under the later two-sided attack rule (`m1a-envelope-score-v2`), the attack rows above are unqualified raw readings rather than failures, because two of the three reference fits are outside the estimator's domain. The Gain result is unaffected. Candidate WAV SHA-256: `628a6a312288ed8dc817be0152329cc9de2fe3ff6e239d987c7f80423a2ba61c`. The rescore was rerun under the qualified estimator (`m1a-envelope-score-v1`, 2026-09-23 build host): attack is unchanged by construction — output volume cannot touch a timing measurement — and the harmonic vector moves only in the seventh digit.
