...
def measure(cutoffs: list[int]) -> dict:
    ...
    for cutoff in cutoffs:
        ...
        patch = {**base_patch, "cutoff": (cutoff, cutoff)}
            ...
            events.append({"midi": note, "reference_f0_hz": round(fref.value, 4),
                           ...
                           "reference": {"alias_db": round(float(ref_alias.value), 4),
                                         "harmonics_db": {f"h{k}": ref_sig.get(f"h{k}")
                                                          for k in range(2, 13)}}})
        outputs.append({"cutoff_hz": cutoff, "events": events})
    ...
    return {
        ...
        "analysis_version": "m5a-signal-path-v1",
        ...
        "cutoff_intervention": "change only model cutoff; all other patch fields fixed",
        "runs": outputs,
    }
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--cutoff", type=int, nargs="+", default=[14073, 21600],
                    help="one or more candidate cutoff values in Hz (default: baseline and maximum)")
