...
module decimate_2x_tm_sym(
 ...
 reg signed [47:0] acc; reg signed [32:0] prod; integer i;
 initial begin
 ...
 always @(posedge clk) begin
  ...
  if(!rst_n) begin parity<=0;busy<=0;tap<=0;acc<=0;out_sample<=0;for(i=0;i<31;i=i+1)x[i]<=0; end
   ...
   if(tap<15) prod=h[tap]*(x[tap]+x[30-tap]); else prod=h[15]*x[15];
   if(tap==15) begin out_sample<=(acc+prod)>>>15;out_valid<=1;busy<=0;end
